...
operator_metadata=""
print_agent="svpchain-perps-agent"  # which agent --print-config renders
...
# at call time.
render_agent_toml() {
  local name="${1:-svpchain-perps-agent}"
...
if [[ "$mode" == "print-config" ]]; then
  # Preview one agent's config (default svpchain-perps-agent; --print-agent
...
if [[ "$mode" == "print-compose" ]]; then
  ...
  resolve_operator_keys
  image_ref="ghcr.io/svpchain/svpchain-perps-agent:${image_tag:-<tag>}"
  render_compose_yaml
  exit 0
fi
...
image_ref="ghcr.io/svpchain/svpchain-perps-agent:${image_tag}"
image_tar="${REPO_DIR}/build/svpchain-perps-agent.image.tar"
...
if [[ "$skip_build" == "1" ]]; then
  ...
  build_cmd+=" -t ghcr.io/svpchain/svpchain-perps-agent:latest"
  build_cmd+=" -f cmd/svpchain-perps-agent/Dockerfile ."
...
compose_tmp="$(mktemp -t svpchain-perps-agent.compose.XXXXXX)"
routes_tmp=""
...
if [[ -n "$bridge_routes_basename" ]]; then
  if [[ -n "$bridge_routes_src_abs" ]]; then
    ...
    routes_tmp="$(mktemp -t svpchain-perps-agent.routes.XXXXXX)"
...
run_or_print "rsync -avz '$image_tar' '$host:$install_dir/svpchain-perps-agent.image.tar'"

# Phase 4: load (On remote)
step "On remote: docker load (skipped if image already loaded)"
load_if_missing "$image_ref" "$install_dir/svpchain-perps-agent.image.tar" "$expected_id"
remote_exec "docker tag $image_ref ghcr.io/svpchain/svpchain-perps-agent:latest"
