...
# only when a key is configured; see the removal of a stale one at the ship
# step, which is what makes unsetting the key actually turn the skill off.
#
# ★ Deleted by accident in cd2ace0, when the splice that shrank
# render_agent_toml ran to the next function's comment and swallowed this one
# in between. The call site survived, so an install with a model key configured
# died at the staging step with "command not found" -- and only then, because
# the call is guarded by the key being set, so every keyless deploy and every
# --print-* mode kept working. TestEveryRenderFunctionCalledIsDefined now
# catches that shape.
